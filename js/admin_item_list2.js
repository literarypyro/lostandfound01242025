var requestModule=angular.module('itemsApp',['datatables','ngSanitize','ngCookies','ngAnimate']);
...
//	$scope.request_id=request_id;
    ...
    // Pagination settings
    $scope.pagination = {
        found: { page: 1, pageSize: 10 },
        expired: { page: 1, pageSize: 10 },
        disposed: { page: 1, pageSize: 10 }
    };
...
/*
	var url=host+"expired";
		...
		.then(function(resp, status, headers, config) {
			...
			//if login is illegal
			$scope.expired_items2=response;
			$scope.expired_items = Object.fromEntries(Object.entries(response).slice(0,6));
	...
	$scope.setItemIndex = function(index) {
...
*/
	...
	$scope.dtOptions=[
		...
		.then(function(resp, status, headers, config) {
			...
			//if login is illegal
			const disposed_key=Object.keys(response)[1];

			$scope.disposed_items2=response[disposed_key];

			$scope.disposed_items = Object.fromEntries(Object.entries($scope.disposed_items2).slice(0,6));
...
//		var url=host+"recent";
	...
	//		const recent_key=Object.keys(response)[1];

//			$scope.found_items = response[recent_key];
//	        $scope.itemData =  Object.keys($scope.items).map(function (key) { return $scope.items[key]; });
//			$scope.itemList=Object.fromEntries(Object.entries($scope.found_items).slice(0,6));
			//console.log(data);
...
//			$scope.itemList = response[recent_key];
//			$scope.itemList=Object.fromEntries(Object.entries($scope.items));
...
$scope.updateFilteredLists = function() {
    ...
    $scope.foundItems = $scope.itemList.filter(function(item) {
        // Try parsing to number for comparison
        return item && item.latest_stat && 
               parseInt(item.latest_stat.status_type_id) === 1;
    });
...
$http.get(url, { cache: true })
    .then(function(resp) {
        $scope.itemList = resp.data;
		        // Find and log one item with status_type_id=1
        $scope.updateFilteredLists(); // Update the filtered lists after data loads
    });
  ...
  // Helper functions
  $scope.getPageArray = function(totalItems, pageSize) {
    var pages = [];
  ...
  $scope.getTotalPages = function(items, pageSize) {
