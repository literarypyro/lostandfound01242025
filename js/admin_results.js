var requestModule=angular.module('itemResultsApp',['datatables','ngSanitize','ngCookies','ngAnimate']);
requestModule.controller("itemResultsController",['$compile', '$scope','$http','$window','$cookies', 'DTOptionsBuilder', 'DTColumnDefBuilder', function itemResultsController($compile, $scope,$http,$window,$cookies, DTOptionsBuilder, DTColumnDefBuilder){
...
//	$cookies.put("A","a");
	...
	$scope.fromsearch=JSON.parse($cookies.get("fromsearch"));
	$scope.isBase=isBase;
...
// When retrieving in the other controller
...
$scope.updateFilteredLists = function() {
    ...
    if($scope.fromsearch){
		...
		$scope.foundItems=$scope.itemList;
...
try {
   $scope.itemList = JSON.parse(sessionStorage.getItem('itemList')) || [];
   $scope.updateFilteredLists();
} catch(e) {
